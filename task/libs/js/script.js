	...
	$('#btnSubmitTwo').click(function() {
		...
		$.ajax({
			...
			success: function(result) {
				...
				console.log(JSON.stringify(result['data']['distance'] + " || " + result['data']['geonameId'] + " || " + result['data']['name']));

				if (result.status.name == "ok") {

				$('#txtResults').html(JSON.stringify(result['data']['distance'] + " || " + result['data']['geonameId'] + " || " + result['data']['name']));
